...
<section class="content mt-4">
    <div class="container-fluid">
        <div class="fade-in">
            <div class="card">
                <h5 class="card-header">Danh sách cơ sở vật chất</h5>
                <?php
                                    ...
                                    <table class="table table-striped table-bordered datatable dataTable no-footer" id="table-search" role="grid" aria-describedby="DataTables_Table_0_info" style="border-collapse: collapse !important">
                                        ...
                                        @foreach( $facilities as $key =>$v)
                                            <tr role="row" class="odd">
                                                <td>{{ $v->Facilities_name }}</td>
                                                <td>{{ $v->Facilities_brand }}</td>
                                                <td>{{ $v->Facilities_quantity }}</td>
                                                <td>
                                                    <a class="btn btn-sm btn-primary" href="{{URL::to('/edit-infrastructure/'.$v->Facilities_id)}}" >
                                                        <i class="fas fa-tools"></i>
                                                    </a>
                                                    <a class="btn btn-sm btn-danger"  href="{{URL::to('/delete-infrastructure/'.$v->Facilities_id)}}" onclick="return confirm('Bạn có chắc chán muốn xóa không?')">
